...
if SERVER then
        ...
        if ct >= (self.nextTick or ct) then
            ...
            if self:GetActive() then
                ...
                elseif def.type == "consumer" then
                    -- Alimentación por enlace: si tiene algún generador enlazado y activo -> tensión nominal, si no -> 0
                    local powered = false
                    for genIdx, map in pairs(ix.power.links or {}) do
                        if map[self:EntIndex()] then
                            local gen = Entity(genIdx)
                            if IsValid(gen) and gen:GetActive() and (gen:GetVoltage() or 0) > 0 then powered = true; break end
                        end
                    end
                    self:SetStored(math.max((self:GetStored() or 0) - math.abs(def.output or 0), 0))
                    self:SetVoltage(powered and voltage or 0)
    ...
    function ENT:Draw()
        self:DrawModel()
        -- Luz dinámica para lámparas activas con tensión
        local id = self:GetDeviceID()
